...
## 使用 shardingjdbc 时报 `java.sql.SQLFeatureNotSupportedException`

由于`ShardingDataSource`并没有正确实现`DataSource`接口，很多方法都是直接`throw new SQLFeatureNotSupportedException("xxx");`，而新版 SpringBoot 在进行健康检查时会使用`DataSourceHealthIndicator`进行，进而调用了`ShardingDataSource`尚未实现的方法导致错误。

解决思路：由于`ShardingDataSource`只是真实`DataSource`的代理，所以可以不直接对`ShardingDataSource`进行健康检测，因为从理论上讲，只要它依赖的真实数据源没有问题，所以它自己也没有问题，所以进行健康检查的时候仅对真实数据源进行健康检查即可。

解决方案：假设项目使用的真实数据源是`HikariDataSource`，则创建一个自定义的`DataSourceHealthIndicator` Bean 即可：

```java
@Bean("dbHealthContributor")
public DataSourceHealthIndicator dbHealthContributor(DataSource dataSource) {
    if (dataSource instanceof ShardingDataSource) {
        final Collection<DataSource> values = ((ShardingDataSource) dataSource).getDataSourceMap().values();
        if (values.isEmpty()) {
            return new DataSourceHealthIndicator();
        }
        // 这里可能有多个真实数据源，如果这些数据源对应的底层数据库是同一个，则随便取一个进行健康检查即可，否则可能需要自己实现 AbstractHealthIndicator 对所有数据库对应的底层数据源进行检查
        for (DataSource value : values) {
            return new DataSourceHealthIndicator(value);
        }
    }
...
> 注意：根据`org.springframework.boot.actuate.autoconfigure.jdbc.DataSourceHealthContributorAutoConfiguration#dbHealthContributor`源吗可知，自定义的 Bean 名称必须是`dbHealthIndicator`或`dbHealthContributor`。且目前的实现忽略了`management.health.db.ignoreRoutingDataSources`配置，如果用到该配置需要根据实际情况修改。
