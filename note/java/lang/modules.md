...
模块系统希望一切都是模块，因为那样它可以统一应用其规则，但与此同时，也不强制模块化（并且这样做不会向后兼容）。调和这两个看似矛盾的要求的机制是[未命名模块](https://dev.java/learn/modules/unnamed-module/)。未命名模块包含类路径中的所有类，并应用了一些特殊规则，但一旦完成，它就可以像任何其他模块一样工作。
...
- 一个名字：未命名的模块没有（有意义，对吧？），这意味着没有其他模块可以在其声明中提及它（例如要求它） 
- dependencies：未命名的模块读取进入图中的所有其他模块 
- exports：未命名模块导出它的所有包并 [打开它们进行反射 ](https://dev.java/learn/modules/opening-for-reflection/)

与未命名模块相反，所有其他模块都被称为 *已命名模块* 。  [服务 ](https://dev.java/learn/modules/services/) 提供的 `META-INF/services`提供给 `ServiceLoader`. 

虽然不是很简单，但未命名模块的概念是有道理的。 这里有有序的模块图，而在那边，稍偏一点的地方，类路径杂乱无章，混杂在它自己的具有一些特殊属性的免费模块中。 

## 阶级道路的混乱 

未命名模块的主要目标是捕获类路径内容并使其在模块系统中工作。 由于类路径上的 JAR 之间从来没有任何界限，现在建立它们没有任何意义，因此整个类路径只有一个未命名的模块。 在其中，就像在类路径上一样，所有公共类都可以相互访问，并且包可以跨 JAR 拆分。 

未命名模块的独特作用及其对向后兼容性的关注赋予了它一些特殊属性。   一个是间歇性地访问 [Java 9 到 16 中的强封装 API ](https://dev.java/learn/modules/strong-encapsulation/) 。   另一个是它没有暴露给应用于命名模块的许多检查。   因此，不会发现在它和其他模块之间拆分的包，并且类路径部分根本不可用。   （这意味着如果命名模块中也存在相同的包，则您可能会因缺少实际存在于类路径中的类而出错。） 

一个有点违反直觉且容易出错的细节是未命名模块的确切构成。   模块化 JAR 变成模块似乎很明显，因此普通 JAR 进入未命名模块，对吧？   但事实并非如此，未命名模块负责 *类路径上的所有 JAR*  ，无论是否模块化。   因此，模块化 JAR 不一定要作为模块加载！   因此，如果一个库开始交付模块化 JAR，它的用户绝不会被迫将它们作为模块使用。   他们可以将它们留在类路径中，在那里它们的代码被捆绑到未命名的模块中。   这允许生态系统几乎相互独立地模块化。 
...
从类路径启动时，输出为 `Module name: null`，表明该类最终出现在未命名的模块中。 从模块路径启动时，您会得到预期的 `Module name: $MODULE`， 在哪里 `$MODULE`是你给模块的名字。 

## 未命名模块的模块解析 

未命名模块与模块图其余部分的关系的一个重要方面是它可以读取哪些其他模块。   如前所述，这些都是使它进入图表的原因。   但是那是哪些模块呢？   从 [模块系统基础知识 ](https://dev.java/learn/modules/intro/)中记住，模块解析通过从根模块（特别是初始模块）开始构建模块图，然后迭代地添加它们的所有直接和传递依赖项。   如果正在编译的代码或应用程序的代码，那将如何工作 `main`方法在未命名模块中，就像从类路径启动应用程序时一样？ 毕竟，纯 JAR 不表达任何依赖关系。 

如果初始模块是未命名的模块，则模块解析将从一组预定义的根模块开始。 根据经验，这些是在运行时找到的模块，但实际规则更详细一些： 
...
- 成为根的一组精确的 
  ...
  模块的存在（即表示整个 Java SE API 的模块；它存在于完整的 JRE 映像中，但可能不存在于创建的自定义运行时映像中和 
  ...
  - 如果 *java.se* 是可观察的，它就成为根。 
  - 如果不是，则每个 *java.** 导出至少一个包的 [无条件 ](https://dev.java/learn/modules/qualified-exports-opens/)模块都会成为根。 

- 除了 *java.** 模块之外，运行时中不是孵化模块并且至少导出一个没有限定的包的每个其他模块都成为根模块。   这与 *jdk.** 模块特别相关。 

- 模块 [列出的 `--add-modules`](https://dev.java/learn/modules/add-modules-reads/)始终是根模块。 

请注意，将未命名模块作为初始模块，根模块集始终是运行时映像中包含的模块的子集。 模块路径上存在的模块将永远不会被解析，除非明确添加 `--add-modules`. 如果您手工制作模块路径以包含您需要的模块，您可能希望添加所有这些模块 `--add-modules ALL-MODULE-PATH`如 [本文 ](https://dev.java/learn/modules/add-modules-reads/)所述。 
