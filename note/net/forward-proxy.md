...
- 选择国外服务器搭建时，一定要注意设置安全组规则或者防火墙，只允许自己所在网络的出口IP访问（直接百度“ip”就能看到自己的出口IP，该方法还可以用于验证代理是否生效），因为有很多公司或个人有专业的服务器来扫描免费代理，一旦会被别人扫描到则可能产生高昂的流量费用，如果发现流量或宽带使用不合理，那多半就是这个原因了。
...
默认情况下，如果给浏览器或者系统设置代理后，则几乎所有请求都会被代理，只有像localhost或127.0.0.1这样的特殊且极少数的请求不会走代理（也可以配置都走），但我们的需求是只有极少数需要代理的部分才使用代理，其他的不使用。在浏览器中一般通过插件来实现，常用插件有`SmartProxy`和`Proxy SwitchyOmega`等。
