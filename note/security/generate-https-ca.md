...
但是不同的客户端添加方式不同，并且就算同一电脑上也可能需要在多个地方添加，比如添加到系统证书列表可以正常使用`curl`等，但是不能使用Firefox和Chromium等，java也是一样要单独添加。

具体详细的添加方式（包括各个系统的工具安装等）可以参数[mkcert](https://github.com/FiloSottile/mkcert)项目的源代码或者直接使用该工具的`mkcert -install`命令添加，以下记录Ubuntu的添加方式。

### 系统（Ubuntu）

添加后可以使用大部分工具，比如`cfssl`等，但是浏览器可能还是会报不安全。
...
2. 更新系统证书
   ...
   `sudo update-ca-certificates`命令会将新增的证书内容附加到`/etc/ssl/certs/ca-certificates.crt`。

   同理，删除对应的证书并执行该命令即可移除对应的证书。
   
### NSS（Ubuntu）

添加后可以使用大部分浏览器，添加时注意`~/.pki/nssdb`、`~/snap/chromium/current/.pki/nssdb`和`/etc/pki/nssdb (CentOS 7)`这些地方以及浏览器安装的位置，如果这些位置下面有`cert9.db`或者`cert8.db`文件的话，既要使用 certutil 工具将根CA添加进去，并且`cert9.db`和`cert8.db`添加方式一样。

```shell
# 如果 ~/.pki/nssdb 下面存在 cert9.db 则执行以下命令，注意要使用全路径，-n 为证书名字，可以随意
$ certutil -A -d sql:/home/laeni/.pki/nssdb -t "C,," -n "Laeni Global Root CA" -i ca.pem
# 如果 ~/.pki/nssdb 下面存在 cert8.db 则执行以下命令，注意要使用全路径，-n 为证书名字，可以随意
$ certutil -A -d dbm:/home/laeni/.pki/nssdb -t "C,," -n "Laeni Global Root CA" -i ca.pem
```

正常将证书添加到证书库之后应该就能在浏览中正常访问该域名的网站了。

### Java

如果需要在java中访问该证书签发的https API时需要将证书添加到java密钥库。

```shell
$ javaHome=/opt/apps/java/jdk-11.0.12+7

$ sudo $javaHome/bin/keytool -importcert -noprompt -keystore $javaHome/lib/security/cacerts -storepass changeit -file ca.pem -alias "Laeni Global Root CA"
$ sudo $javaHome/bin/keytool -importcert -noprompt -keystore $javaHome/jre/lib/security/cacerts -storepass changeit -file ca.pem -alias "Laeni Global Root CA"
```

## 问题排查

如果进行如上操作后还是提示证书不受信任的话，可以参考以下进行排查。

1. 先直接使用`curl`访问，如果curl能访问，但是浏览器不能正常访问则是没有将根证书添加到浏览器的证书库中。

2. 如果连`curl`也也不能访问则是证书生成错误或者配置错误。

   可以先用`openssl`验证相邻的两个证书中父证书是否能验证子证书（注意不能跨级验证）：

   ```shell
   $ openssl verify -verbose -CAfile 父证书 子证书
   ```

   也可以使用`curl`指定证书进行验证，以此来确定是哪个步骤的问题：

   ```shell
   # 这里使用中间CA和根CA都应该能通过
   $ curl --cacert 中间CA https://xx.laeni.cn:1443
   $ curl --cacert 根CA https://xx.laeni.cn:1443
   ```
